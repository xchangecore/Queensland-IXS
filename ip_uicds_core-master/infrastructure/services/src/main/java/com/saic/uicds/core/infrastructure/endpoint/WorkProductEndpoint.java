...
public class WorkProductEndpoint
    ...
    public ArchiveProductResponseDocument archiveProduct(ArchiveProductRequestDocument request)
        ...
        response.addNewArchiveProductResponse().set(
            WorkProductHelper.toWorkProductProcessingStatus(productService.archiveProduct(request.getArchiveProductRequest().getWorkProductIdentification())));
        return response;
    }
    ...
    public AssociateWorkProductToInterestGroupResponseDocument associateWorkProductToInterestGroup(
        AssociateWorkProductToInterestGroupRequestDocument request) throws SOAPException {
        ...
        String wpID = productService.associateWorkProductToInterestGroup(
            request.getAssociateWorkProductToInterestGroupRequest().getWorkProductID().getStringValue(),
    ...
    public CloseProductResponseDocument closeProduct(CloseProductRequestDocument request)
        ...
        response.addNewCloseProductResponse().addNewWorkProductPublicationResponse().set(
            WorkProductHelper.toWorkProductPublicationResponse(productService.closeProduct(request.getCloseProductRequest().getWorkProductIdentification())));
        return response;
    }
    ...
    public GetAssociatedWorkProductListResponseDocument getAssociatedWorkProductList(
        GetAssociatedWorkProductListRequestDocument request) throws DatatypeConfigurationException {
        ...
        if (products != null) {
            ...
            for (WorkProduct product : products) {
                response.getGetAssociatedWorkProductListResponse().getWorkProductList().addNewWorkProduct().set(
    ...
    public GetProductResponseDocument getProduct(GetProductRequestDocument request)
        ...
        WorkProductDocument.WorkProduct theWorkProduct = WorkProductHelper.toWorkProduct(productService.getProduct(request.getGetProductRequest().getWorkProductIdentification()));
    ...
    public GetProductCurrentVersionResponseDocument getProductCurrentVersion(
        GetProductCurrentVersionRequestDocument request) throws SOAPException {

        GetProductCurrentVersionResponseDocument response = GetProductCurrentVersionResponseDocument.Factory.newInstance();
        WorkProductDocument.WorkProduct theWorkProduct = WorkProductHelper.toWorkProduct(productService.getProduct(request.getGetProductCurrentVersionRequest().getIdentifier().getStringValue()));
        if (theWorkProduct != null) {
    ...
    public PublishProductResponseDocument publishProduct(PublishProductRequestDocument request)
        ...
        // Get workProduct
        WorkProduct product = WorkProductHelper.toModel(request.getPublishProductRequest().getWorkProduct());

        if(product.getAssociatedInterestGroupIDs() == null || (product.getAssociatedInterestGroupIDs() 
        		!= null && product.getAssociatedInterestGroupIDs().size() == 0)) {
	    	if (request.getPublishProductRequest().getIncidentId() != null) {
	    		Set<String> associatedInterestGroupIDs = new HashSet<String>();
	    		associatedInterestGroupIDs.add(request.getPublishProductRequest().getIncidentId());
				product.setAssociatedInterestGroupIDs(associatedInterestGroupIDs);
	    	}
        }
        // Publish Product
        ProductPublicationStatus status = productService.publishProduct(product);

        // Create Response
        PublishProductResponseDocument response = PublishProductResponseDocument.Factory.newInstance();
        response.addNewPublishProductResponse().addNewWorkProductPublicationResponse().set(
