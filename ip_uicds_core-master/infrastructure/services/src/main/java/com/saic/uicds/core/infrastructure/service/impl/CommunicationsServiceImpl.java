...
import com.saic.uicds.core.infrastructure.service.CommunicationsInterestGroupSharingService;
...
public class CommunicationsServiceImpl
    ...
    private HashMap<String, String> serviceProductTypeMap = new HashMap<String, String>();

    // Key: interestGroupID
    /** The interest group list. */
    private List<String> interestGroupList = new ArrayList<String>();
    ...
    /* (non-Javadoc)
     * @see com.saic.uicds.core.infrastructure.service.CommunicationsService#sendXMPPMessage(java.lang.String, java.lang.String, java.lang.String, java.lang.String)
     */
    @Override
    ...
    public void handleInterestGroupState(InterestGroupStateNotificationMessage message) {
        ...
        logger.info("####### receiving interest group state notification for interest group:"
            + message.getInterestGroupID() + " state:" + message.getState() + " sharingStatus="
            + message.getSharingStatus() + " type=" + message.getInterestGroupType());
        ...
        if (message.getState().equals(InterestGroupStateNotificationMessage.State.NEW)
            ...
            if (!interestGroupList.contains(message.getInterestGroupID())) {
                ...
                Message<NewInterestGroupCreatedMessage> requestMessage = (Message<NewInterestGroupCreatedMessage>) new GenericMessage<NewInterestGroupCreatedMessage>(
                    request);
        ...
        if (associationType == ProductToInterestGroupAssociationMessage.AssociationType.Associate) {
            ...
            if (interestGroupList.contains(interestGroupID)) {
                ...
                ProductPublicationMessage notification = new ProductPublicationMessage(
                    ProductPublicationMessage.PublicationType.Delete, interestGroupID, productID,
                    productType, null);
    ...
    public void newWorkProductVersion(String workProductID, Integer subscriptionId) {
        ...
        logger.debug("newWorkProductVersion - receiving publication of workProductID:"
            + workProductID + " subscriptionId:" + subscriptionId);
        ...
        if (wp != null) {
            ...
            logger.debug("newWorkProductVersion - sending ProductPublicationMessage with interest groupID="
                + interestGroupID
                + " wpID="
                + wpID
                + "  pubStatus="
            ...
            ProductPublicationMessage notification = new ProductPublicationMessage(
                ProductPublicationMessage.PublicationType.Publish, interestGroupID, wpID, wpType,
                wpString);
            Message<ProductPublicationMessage> msg = new GenericMessage<ProductPublicationMessage>(
                notification);
            productPublicationChannel.send(msg);
        } else {
            logger.error("newWorkProductVersion - unable to retrieve work product with wpID="
                + workProductID + " from workProductService");
        }
    ...
    public void joinedPublishProductNotificationHandler(JoinedPublishProductRequestMessage message) {
        ...
        if (!requestingCore.equals(configurationService.getCoreName())) {
            ...
            ProductPublicationStatus status = workProductService.publishProducRequesttFromJoinedCore(
    ...
    public void productPublicationStatusNotificationHandler(
        ...
        try {
            ...
            logger.debug("===> sending notification message: array size=" + notification.length);
