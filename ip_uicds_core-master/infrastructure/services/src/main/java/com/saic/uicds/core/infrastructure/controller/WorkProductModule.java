...
public class WorkProductModule extends StaxCompositeModule {
	...
	private static class ResultsFormatter extends StaxFormatter {

		@Override
		public Object format(MarshalContext context, Object input,
				ExtendedXMLStreamWriter writer) throws MarshalException,
				XMLStreamException {
			...
			if (results.isPaging()) {
				writer
						.element(OPENSEARCH.ITEMS_PER_PAGE, results
								.getPageSize());
			}
			...
			for (Object result : results) {
				if (result instanceof Entity) {
					...
					Geometry geometry = (Geometry) ((Entity) result)
							.getValue(GEORSS.WHERE);
					if (geometry != null) {
						Envelope env = geometry.getEnvelopeInternal();
						if (bbox == null)
							bbox = new Envelope(env.getMinX(), env.getMaxX(),
		...
		void formatEntity(MarshalContext context, Entity entity,
				ExtendedXMLStreamWriter out) throws XMLStreamException {
			...
			out.element(ATOM.ENTRY);
			{
				...
				out.attribute("scheme",
						"http://uicds.org#types");
		...
		protected String getId(Object value) {
			...
			for (Method method : methods) {
				if ("getId".equals(method.getName())) {
					try {
						return Coerce.toString(method.invoke(value,
								new Object[0]));
					} catch (Exception e) {
					}
				}
