...
public class WorkProductHelper
     ...
     * @return
     */
    public static final IdentificationType getIdentificationElement(
        ...
        if (workProduct != null && workProduct.getPackageMetadata() != null) {
            XmlObject[] objects = workProduct.getPackageMetadata().selectChildren(
                new QName(NS_PRECIS_STRUCTURES, S_PACKAGE_IDENTIFICATION));
            if (objects.length > 0) {
     ...
     * @return
     */
    public static final PropertiesType getPropertiesElement(
        com.saic.precis.x2009.x06.structures.WorkProductDocument.WorkProduct workProduct) {

        PropertiesType properties = null;
        if (workProduct != null && workProduct.getPackageMetadata() != null) {
            XmlObject[] objects = workProduct.getPackageMetadata().selectChildren(
                new QName(NS_PRECIS_STRUCTURES, S_PACKAGE_PROPERTIES));
            if (objects.length > 0) {
    ...
    public static final DigestType getDigestElement(
        com.saic.precis.x2009.x06.structures.WorkProductDocument.WorkProduct workProduct) {
    ...
    private static final WorkProductDocument toWorkProductDocument(WorkProduct wp, boolean summary) {
        ...
        XmlUtil.substitute(pkgMetadata.addNewDataItemStatusAbstract(), NS_UCORE,
            S_DATA_ITEM_STATUS, DataItemStatusType.type, dataItemStatus);
        ...
        XmlUtil.substitute(ownerMetadata.addNewDataOwnerContactAbstract(), NS_UCORE,
            S_DATA_OWNER_CONTACT, PointOfContactType.type, contact);

        // DataOwnerIdentifier
        contact = PointOfContactType.Factory.newInstance();
        contact.addNewOrganization().addName(URI_UICDS);
        XmlUtil.substitute(ownerMetadata.addNewDataOwnerIdentifierAbstract(), NS_UCORE,
            S_DATA_OWNER_IDENTIFIER, PointOfContactType.type, contact);
        ...
        XmlUtil.substitute(ownerMetadata.addNewDataOwnerMetadataExtensionAbstract(),
            NS_ULEX_STRUCTURE, S_DATA_OWNER_METADATA_DOMAIN_ATTRIBUTE, DomainAttributeType.type,
            domain);

        // DisseminatinCriteria
        XmlUtil.substitute(pkgMetadata.addNewDisseminationCriteriaAbstract(), NS_UCORE,
            S_DISSEMINATION_CRITERIA, DisseminationCriteria.type,
            DisseminationCriteria.Factory.newInstance());

        // WorkProductIdentification
        XmlUtil.substitute(pkgMetadata.addNewPackageMetadataExtensionAbstract(),
            NS_PRECIS_STRUCTURES, S_PACKAGE_IDENTIFICATION, IdentificationType.type, wpid);

        // WorkProductProperties
        XmlUtil.substitute(pkgMetadata.addNewPackageMetadataExtensionAbstract(),
            NS_PRECIS_STRUCTURES, S_PACKAGE_PROPERTIES, PropertiesType.type,
            getWorkProductProperties(wp));

        // Digest part
        if (wp.getDigest() != null) {
            XmlUtil.substitute(theWorkProduct.addNewDigestAbstract(), NS_UCORE, S_DIGEST,
                DigestType.type, wp.getDigest().getDigest());
        ...
        if (!summary) {
            ...
            payload.getStructuredPayloadMetadata().setCommunityVersion(
                wp.getProductTypeVersion() != null ? wp.getProductTypeVersion() : "");
        }

        return wpd;
    }

    public static final WorkProductProcessingStatusDocument toWorkProductProcessingStatus(
        ...
        if (status.getStatus().equals(ProductPublicationStatus.SuccessStatus)) {
            processingStatus.getWorkProductProcessingStatus().setStatus(
                ProcessingStateType.ACCEPTED);
        } else {
            processingStatus.getWorkProductProcessingStatus().setStatus(
                ProcessingStateType.REJECTED);
            processingStatus.getWorkProductProcessingStatus().addNewMessage().setStringValue(
    ...
    public static final WorkProductPublicationResponseDocument toWorkProductPublicatinResponse(
        String statusStr) {
    ...
    public static final WorkProductPublicationResponseType toWorkProductPublicationResponse(
        ProductPublicationStatus status) {
        ...
        if (status.getStatus().equals(ProductPublicationStatus.SuccessStatus)) {
            ...
            theStatus.getWorkProductProcessingStatus().addNewMessage().setStringValue(
                status.getReasonForFailure());
        } else {
            // fill the ACTUuid
