package com.saic.uicds.core.infrastructure.rule;
...
public class Rule {
	...
	public Rule(String componentId, String event, Status beginStatus,
			Status endStatus, String message) {
	...
	private boolean containsAll(String msg) {
		...
		for (String keyWord : keyWords) {
			...
			&& msg.toLowerCase().contains(keyWord.toLowerCase());
		}
		return containsAll;
	}
	...
	public boolean isInState(Status status) {
		if(beginStatus == null) return false;
		boolean name = status.getName().equalsIgnoreCase(
				beginStatus.getName());
		return name;
	}
