		...
		<xsl:choose> 
			<xsl:when test="string-length($EventID_Incident) > 0"> 
				<ns:Event>
					...
					<ns:Identifier ns:label="label">CommitResource - <xsl:value-of select="rmsg:CommitResource/rmsg:ContactInformation/rm:ContactDescription"/></ns:Identifier>
			...
			<xsl:if test="count(rmsg:ScheduleInformation) > 0">
				<ns:Location>
					...
					<xsl:if test="count(rmsg:ScheduleInformation/rmsg:Location/rm:Address) > 0">
						<ns:PhysicalAddress>
							<ns1:postalAddress>
								<ns1:street><xsl:value-of select="rmsg:ScheduleInformation/rmsg:Location/rm:Address/xal:Thoroughfare/xal:Number"/><xsl:text> </xsl:text><xsl:value-of select="rmsg:ScheduleInformation/rmsg:Location/rm:Address/xal:Thoroughfare/xal:NameElement"/>
								</ns1:street>
								<ns1:city><xsl:value-of select="rmsg:ScheduleInformation/rmsg:Location/rm:Address/xal:Locality/xal:NameElement"/></ns1:city>
