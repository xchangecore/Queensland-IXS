...
import com.saic.uicds.core.em.util.DetailedCFSMessageReader;
import com.saic.uicds.core.em.util.DetailedCFSMessageUtil;
 ...
 * 
 * @see com.saic.uicds.core.em.model.LEITSCIncident LEITSCIncident Data Model
        ...
        if (existingLeitscIncident == null) {
            ...
            if ((status != null)
                ...
                log.debug("===> createLEITSCIncident:  uicds incident ID="
                    ...
                    + status.getProduct().getProductID() + " was created for LEITSC incident ID="
                    + leitscIncidentID);
            ...
            if (wp != null) {
                ...
                if (incident != null) {
                    ...
                    String reasonForFailure = " leitscIncident with ID" + leitscIncidentID
                        ...
                        + leitscIncident.getIncidentID()
                        + " failes to be generated from the work product wpID=" + wp.getProductID();
                    log.error("postDetailedCFSMessage - failed to open leitsc incident "
                        + leitscIncidentID + " reason=" + reasonForFailure);
                    throw new LEITSCIncidentPublicationException(leitscIncidentID, "open",
                        reasonForFailure);
                ...
                String reasonForFailure = " leitscIncident with ID" + leitscIncidentID
                    + " whose UICDS incident with incident ID=" + leitscIncident.getIncidentID()
