...
public class WorkProductFeedUtil implements InfrastructureNamespaces, DigestConstant {
    ...
    private static Set<Entry> getRequestResourceFeedEntries(WorkProduct workProduct,
        ...
        Set<ThingType> things = EMDigestHelper.getThingsByWhatType(digest, NS_UCORE_CODESPACE,
        ...
        if (things != null && things.size() > 0) {
            for (ThingType thing : things) {
                if (thing instanceof EventType) {
                    ...
                    SimplePropertyType resourceProperty = EMDigestHelper.getSimplePropertyFromThing(
                        thing, "http://nimsonline.org/2.0", null, "Resource", null);
                    SimplePropertyType quantityProperty = EMDigestHelper.getSimplePropertyFromThing(
                        thing, "urn:oasis:names:tc:emergency:EDXL:RM:1.0", null, "Quantity", null);
    ...
    private static Geometry getRequestLocation(DigestType digest, String id) {
        ...
        EventLocationRelationshipType locatedAt = EMDigestHelper.getLocationRelationshipByTypeAndEventID(
            digest, S_OccursAt, cause.getCause());
            ...
            for (XmlObject element : elements) {
                if (element instanceof EntityType) {
                    ...
                    if (entityIds.keySet().contains(entity.getId())) {
                        ...
                        entry.put(
                            GEORSS.WHERE,
                            EMDigestHelper.getGeometryFromLocationByID(digest,
    ...
    private static Entry createWorkProductFeedEntry(WorkProduct workProduct, String linkPrefix) {
        ...
        entry.getLinks().add(
            new Link("WorkProduct", linkPrefix + workProduct.getProductID(), "rel", ""));
