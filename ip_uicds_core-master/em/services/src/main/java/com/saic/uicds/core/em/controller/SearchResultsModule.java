...
import com.saic.uicds.core.em.util.WorkProductFeedUtil;
import com.saic.uicds.core.infrastructure.model.WorkProduct;
import com.saic.uicds.core.infrastructure.util.DigestHelper;

public class SearchResultsModule extends StaxCompositeModule {

    public void configure(Configuration cfg) {

        super.configure(cfg);
        addFinder(new StaxFormatterFinder(new ResultsAtomFeedFormatter(), Results.class).property(
            "format", "atom"));
        addFinder(new StaxFormatterFinder(new ResultsRssFeedFormatter(), Results.class).property(
            "format", "rss"));
        addFinder(new StaxFormatterFinder(new ResultsKmlFeedFormatter(), Results.class).property(
            "format", "kml"));
        cfg.require(StaxAdaptersModule.class);
    }

    private static class ResultsAtomFeedFormatter extends StaxFormatter {
    ...
    private static class ResultsRssFeedFormatter extends StaxFormatter {
        ...
        void formatEntity(MarshalContext context, WorkProduct product, ExtendedXMLStreamWriter out)
            ...
            for (Entry entry : entries) {
                out.element(RSS2.ITEM);
                {
                    ...
                    out.element("precisb", "AssociatedGroups",
    ...
    private static class ResultsKmlFeedFormatter extends StaxFormatter {
        ...
        private void addWMSElements(ExtendedXMLStreamWriter writer, Results<?> results,
            String baseURL) {
        ...
        private void addWMSElement(ExtendedXMLStreamWriter writer, WorkProduct map, String baseURL)
            ...
            try {
                ...
                ViewContextDocument viewContext = ViewContextDocument.Factory.parse(new String(
                    map.getProduct().xmlText()));
