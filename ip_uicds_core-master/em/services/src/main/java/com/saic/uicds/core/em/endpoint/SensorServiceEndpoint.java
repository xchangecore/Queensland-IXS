 ...
 */
@Endpoint
public class SensorServiceEndpoint implements ServiceNamespaces {
	@Autowired
	...
	public CreateSOIResponseDocument CreateSOI(
			CreateSOIRequestDocument requestDoc) {

		CreateSOIResponseDocument responseDoc = CreateSOIResponseDocument.Factory
				.newInstance();
		SensorObservationInfo soi = requestDoc.getCreateSOIRequest()
				.getSensorObservationInfo();
		String incidentID = requestDoc.getCreateSOIRequest().getIncidentID();
		System.out.println("CreateSOIResponseDocument: soi=[" + soi.toString()
				+ "] incidentID=" + incidentID);
		ProductPublicationStatus status = sensorService.createSOI(soi,
				incidentID);

		responseDoc
				.addNewCreateSOIResponse()
	...
	public UpdateSOIResponseDocument UpdateSOI(
			UpdateSOIRequestDocument requestDoc) {

		UpdateSOIResponseDocument responseDoc = UpdateSOIResponseDocument.Factory
				.newInstance();
		ProductPublicationStatus status = sensorService.updateSOI(requestDoc
				.getUpdateSOIRequest().getSensorObservationInfo(), requestDoc
				.getUpdateSOIRequest().getWorkProductIdentification());
		responseDoc
				.addNewUpdateSOIResponse()
				.addNewWorkProductPublicationResponse()
				.set(WorkProductHelper.toWorkProductPublicationResponse(status));
		return responseDoc;
	}
	...
	public GetSOIResponseDocument GetSOI(GetSOIRequestDocument requestDoc) {
		...
		IdentificationType workProductIdentification = request
				.getWorkProductIdentification();

		GetSOIResponseDocument responseDoc = GetSOIResponseDocument.Factory
				.newInstance();
		GetSOIResponse response = responseDoc.addNewGetSOIResponse();
		response.setWorkProduct(WorkProductHelper.toWorkProduct(sensorService
				.getSOI(workProductIdentification.getIdentifier()
						.getStringValue())));
		return responseDoc;
	}
	...
	public GetSOIListResponseDocument GetSOIList(
			GetSOIListRequestDocument requestDoc) {

		GetSOIListRequest getSOIListRequest = requestDoc.getGetSOIListRequest();

		WorkProduct[] products = sensorService.getSOIList(getSOIListRequest
				.getIncidentID());
		...
		if (products != null && products.length > 0) {
			...
			for (WorkProduct product : products) {
				workProductSummaryArray[i] = WorkProductHelper
						.toWorkProductSummary(product);
				i++;
			}
		}

		GetSOIListResponseDocument responseDoc = GetSOIListResponseDocument.Factory
				.newInstance();
		responseDoc.addNewGetSOIListResponse().addNewWorkProductList()
				.setWorkProductArray(workProductSummaryArray);
