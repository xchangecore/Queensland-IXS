...
import com.saic.uicds.core.em.util.GetCapabilitiesHandler;
import com.saic.uicds.core.em.util.MapRenderer;
import com.saic.uicds.core.em.util.XmfView;
...
public class MapServiceWmsController extends AbstractController {
    ...
    private ModelAndView handleGetMap(HttpServletRequest request, HttpServletResponse response)
        ...
        // render the map
        Image image = renderer.render((Element) viewContext, layerNames, envelope, width, height,
                format);
    ...
    private ModelAndView handleGetCapabilities(HttpServletRequest request,
        ...
        ServiceMetadata capabilities = handler.computeMetadata((Element) viewContext,
                (GetCapabilitiesRequest) owsRequest, request);
    ...
    private Envelope getBoundingBox(HttpServletRequest request, Node viewContext) {
        ...
        if (StringUtils.isNotEmpty(bboxStr)) {
            ...
            envelope = new Envelope(Double.parseDouble(bbox[0]), Double.parseDouble(bbox[2]),
    ...
    private Envelope getDefaultBBOX(Node node) {
        ...
        return new Envelope(Coerce.toDouble(bbox.getAttribute("minx")), Coerce.toDouble(bbox
                .getAttribute("maxx")), Coerce.toDouble(bbox.getAttribute("miny")),
