...
public class ShapefileIngester {
	...
	private MarshalContext context = new MarshalContext(AtomModule.class);
	private String DEFAULT_SCALAR = "1";
	private String STYLE_CODESPACE = "http://uicds.us/style";
	...
	public List<WorkProduct> parseFeatures(String incidentId, InputStream in)
			throws Exception {
		...
		for (Entity e : results.list()) {
			...
			if (incidentId != null) {
				feature.getAssociatedInterestGroupIDs().add(incidentId);
			}
			feature.setCreatedDate(new Date());
			feature.setMimeType("application/atom+xml");
			// TODO do we have better way to do this ???
			feature.setProduct(XmlObject.Factory.parse(new String(out
					.toByteArray())));
			out.close();
			...
			ThingType thing = digest.getDigest().getDigest()
					.getThingAbstractArray()[0];
			...
			if (abgr != null) {
				digest.addSimplePropertyToThing(thing, STYLE_CODESPACE,
						"color", "Colors", "#" + abgr);
			}

			// Scalar Simple Property
			digest.addSimplePropertyToThing(thing, STYLE_CODESPACE, "scalar",
					"Scalar", scalar);
			if (where instanceof Point) {
	...
	private void setPolygonDigest(DigestHelper digest, Geometry where) {
		...
		if (linearRing != null) {
			XmlUtil.substitute(gPolygon.addNewExterior().addNewAbstractRing(),
					...
					DigestConstant.S_LinearRing, LinearRingType.type,
					linearRing);
		}
	...
	private void setPointDigest(DigestHelper digest, Geometry where) {
		Point pointGeom = (Point) where.getCentroid();
		...
		pos.setStringValue(pointGeom.getCoordinate().y + " "
				+ pointGeom.getCoordinate().x);
		point.setPos(pos);
