...
public class FeatureWmsController
    ...
    protected ModelAndView handleRequestInternal(HttpServletRequest request,
        ...
        String selected = getParameter("selected", request,
            getParameter("feature.selection", request, ""));
     ...
     * @throws Exception
     */
    protected Image render(List<WorkProduct> features, Envelope bbox, Dimension2D window,
        String format, String selected)
        throws Exception {

        format = format.substring(format.indexOf("/") + 1, format.length());
        BufferedImage image = createImage(format, (int) window.getWidth(),
            (int) window.getHeight(), true);
    ...
    protected void renderFeatures(List<WorkProduct> features, String selected, Graphics2D graphics)
        ...
        for (WorkProduct feature : features) {
            ...
            InputStream in = new ByteArrayInputStream(
                new String(feature.getProduct().xmlText()).getBytes("UTF-8"));
     ...
     * @return Envelope
     */
    private Envelope getBoundingBox(HttpServletRequest request)
        throws Exception {
