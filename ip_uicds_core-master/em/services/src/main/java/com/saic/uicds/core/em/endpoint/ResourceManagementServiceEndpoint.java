...
/**
 * The Resource Management Service provides UICDS clients with services to
 * exchange EDXL-RM messages with other UICDS clients and send EDXL-RM messages
 * to XMPP users. The clients can exchange any of the EDXL-RM messages from any
 * of the EDXL-RM messaging phases: Discovery, Ordering and Deployment. It is up
 * to the clients to implement the actions necessary to support these message
 * exchanges according to the EDXL-RM specification. The UICDS Resource
 * Management Service merely routes these messages to explicitly targeted
 * clients and delivers the message on the clients notification queue or XMPP
 * message queue while maintaining a minimal summary of the message exchanges.
 * The Resource Management service will maintain the following summary
 * information about resources for each incident:
 * <ul>
 ...
 * <ul>
 * <li>Routing any EDXL-RM message to specific UICDS clients based on the
 * EDXL-DE Explicit Address element(s). See use of explicit address usage in
 * {@link com.saic.uicds.core.em.endpoint.BroadcastServiceEndpoint}</li>
 * <li>Routing any EDXL-RM message to specific XMPP JID based on the EDXL-DE
 * Explicit Address element(s). See use of explicit address usage in
 * {@link com.saic.uicds.core.em.endpoint.BroadcastServiceEndpoint}</li>
 * <li>Restricting routing of EDXL-RM messages to only those cores that have
 * valid agreements</li>
 ...
 * </ul>
 * <li>Providing a summary of requested and committed resources for an incident</li>
 * </ul>
 * <p>
 * An example of the interaction of two UICDS clients, in this example two CAD
 * systems, with an UICDS core to support EDXL-RM messaging can be seen in the
 * following sequence diagram. <br/>
 * <img src="doc-files/rm_slide3.jpg"/> <br/>
 * <p>
 * <!-- NEWPAGE -->
 * <p>
 * The main points about working with the EDXL-RM schemas when interacting with
 * UICDS are:
 * <ul>
 * <li>All elements required by the EDXL-RM specification must be represented</li>
 * <li>Where EDXL-RM allows a choice of required elements UICDS may specify a
 * required or preferred element</li>
 * <li>Where EDXL-RM allows multiple representations UICDS may designate a
 * preferred representation</li>
 * <li>UICDS will not change any EDXL-RM message and will preserve all EDXL-RM
 * message content (i.e. optional elements)</li>
 * </ul>
 * <p>
 * Since UICDS only parses the Request and Commit Resource messages it only
 * places requirements on those messages. The following diagrams show details of
 * data requirements for these two messages. The full details of what data is
 * required beyond that required for UICDS needs to be negotiated between the
 * systems that are exchanging EDXL-RM messages. <br/>
 ...
 * <!-- NEWPAGE -->
 * <p>
 * The Request and Commit Resource message are used within UICDS to create a
 * work product and digest so that these data can be searched and provided as
 * outputs. The following diagrams show the digests that are created for the
 * Request and Commit Resource messages within UICDS. Full details of the
 * mapping from the EDXL-RM message to the Digests fields can be found in the
 * CommitResourceDigest.xsl and RequestResourceDigest.xsl XSL Transformation
 * files. The following tables shows the default mappings.
 * <p>
 ...
 * </tr>
 * <tr>
 * <td>CommitResource.ResourceInformation.ScheduleInformation.Location.
 * LocationDescription</td>
 * <td>Location.Descriptor</td>
 * </tr>
 * <tr>
 * <td>CommitResource.ResourceInformation.ScheduleInformation.Location.Address.
 * Thoroughfare.Number
 * CommitResource.ResourceInformation.ScheduleInformation.Location
 * .Address.Thoroughfare.NameElement</td>
 ...
 * <tr>
 * <td>CommitResource.ResourceInformation.ScheduleInformation.Location.Address.
 * PostCode.Identifier</td>
 * <td>Location.PhysicalAddress.postalAddress.postalCode</td>
 * </tr>
 * <tr>
 * <td>CommitResource.ResourceInformation.ScheduleInformation.Location.Address.
 * Country.NameElement@NameCode</td>
 * <td>Location.PhysicalAddress.postalAddress.countryCode</td>
 * </tr>
 * <tr>
 * <td>
 * CommitResource.ResourceInformation.ScheduleInformation.Location.TargetArea
 * .Point.pos</td>
 ...
 * </tr>
 * <tr>
 * <td>RequestResource.ResourceInformation.AssignmentInformation.Quantity.
 * QuantityText</td>
 * <td>Entity.SimpleProperty</td>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td>RequestResource.ResourceInformation.Resource.TypeInfo.MinimumCapabilities
 * </td>
 * <td>Entity.SimpleProperty</td>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td>RequestResource.ResourceInformation.ScheduleInformation.Location.
 * LocationDescription</td>
 * <td>Location.Descriptor</td>
 * </tr>
 * <tr>
 ...
 * <tr>
 * <td>
 * RequestResource.ResourceInformation.ScheduleInformation.Location.TargetArea
 * .Point.pos</td>
 * <td>Location.GeoLocation.Point.Point.pos</td>
 * </tr>
 * <tr>
 ...
 * <!-- NEWPAGE -->
 * <p>
 * As stated in the EDXL-RM specification an EDXL-RM message posted to UICDS
 * must be encapsulated in an EDXL-DE message. Currently UICDS only routes
 * EDXL-DE messages based on values in the explicit address element. The values
 * in this element must have a type of uicds:user with a value of a resource
 * instance identifier.
 * 
 * An example explicit address for RMApplication@core1 to send an EDXL-RM
 * message to RMApplication@core2 would be:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Note that the explicitAddressSchema should be set to "uicds:user" and the
 * explicitAddressValue set to the resource instance identifier that is listed
 * in the agreements and is also the actual identifier that RMApplication2 is
 * using to get notification messages from UICDS. <br/>
 * 
 * An example explicit address to send an EDXL-RM message to an XMPP user would
 * be:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The EDXL-DE message will be delivered to XMPP clients as and XMPP message.
 * The body will be a short summary of the sender id and date/time sent from the
 * EDXL-DE header information. If each ContentObject element contains a
 * ContentDescription element then that text will be included also. The full
 * EDXL-DE message will be a sub-element of the XMPP message element.
 * 
 * Note that the explicitAddressSchema should be set to "xmpp" and the
 * explicitAddressValue set to the Jabber identifier (JID) of the XMPP user.
 * <br/>
 * <p>
 * 
 ...
 * @see <a
 *      href="../../services/ResourceManagement/0.1/ResourceManagementService.xsd">Appendix:
 ...
 * @see <a
 *      href="http://docs.oasis-open.org/emergency/edxl-de/v1.0/EDXL-DE_Spec_v1.0.pdf">OASIS
 *      EDXL-DE Specification</a>
 * @see <a
 *      href="http://docs.oasis-open.org/emergency/edxl-rm/v1.0/EDXL-RM-SPEC-V1.0.pdf">OASIS
 *      EDXL-RM Specification</a>
 * @see com.saic.uicds.core.em.endpoint.BroadcastServiceEndpoint
 * @see com.saic.uicds.core.em.endpoint.NotificationServiceEndpoint
 * @see com.saic.uicds.core.em.endpoint.ResourceInstanceServiceEndpoint
 * @author roger
 * @author Ron Ridgely
 * @idd
 */
...
public class ResourceManagementServiceEndpoint implements ServiceNamespaces {
	...
	Logger log = LoggerFactory
			.getLogger(ResourceManagementServiceEndpoint.class);
	...
	void setResourceManagementService(
			ResourceManagementService resourceManagementService) {
	...
	/**
	 * Allows the client to submit an EDXL-RM document wrapped in EDXL-DE for
	 * delivery to explicit addresses or as information only if it is a
	 * CommitResource or RequestResource. Submitting a CommitResource or
	 * RequestResource without an explicit address will create the appropriate
	...
	public EdxlDeResponseDocument edxldeRequest(EdxlDeRequestDocument requestDoc) {
		...
		EDXLDistribution request = requestDoc.getEdxlDeRequest()
				.getEDXLDistribution();
		EdxlDeResponseDocument response = EdxlDeResponseDocument.Factory
				.newInstance();
		...
		try {
			...
			for (String core : coresWithError) {
				EdxlDeMessageErrorType error = response.getEdxlDeResponse()
						.addNewCoreError();
	...
	/**
	 * Gets a list of RequestResource work product digests. Parsing the digest
	 * elements will yield a list of resources requested for the incident.
	 * 
	...
	public GetRequestedResourcesResponseDocument getRequestedResources(
		...
		GetRequestedResourcesRequest request = requestDoc
				.getGetRequestedResourcesRequest();
		WorkProduct[] workProducts = resourceManagementService
				.getRequestedResources(request.getIncidentID());
	...
	/**
	 * Gets a list of CommitResource work product digests. Parsing the digest
	 * elements will yield a list of resources committed to the incident.
	 * 
	...
	public GetCommittedResourcesResponseDocument getCommittedResources(
		...
		GetCommittedResourcesRequest request = requestDoc
				.getGetCommittedResourcesRequest();
		WorkProduct[] workProducts = resourceManagementService
				.getCommittedResources(request.getIncidentID());
